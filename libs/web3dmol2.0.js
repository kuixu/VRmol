...
w3m.tool = {
        ...
        for ( var i = 1; i < len; i++ ) {
            ...
            for ( var ii = 1; ii <= n; ii++ ) {
				...
				if(w3m.CLENGTH>0){
					...
					tempPathArray.push(new THREE.Vector3(xyz[0]+offset.x,xyz[1]+offset.y,xyz[2]+offset.z));
					// tempIdArray.push(id);
					tempBinormalsArray.push(new THREE.Vector3(binormal[0],binormal[1],binormal[2]));
					tempNormalsArray.push(new THREE.Vector3(normal[0],normal[1],normal[2]));
					tempTangentsArray.push(new THREE.Vector3(tan[0],tan[1],tan[2]));					
					// if(ii==n){
						// var atom = w3m.tool.getMainAtomById(w3m.global.mol,id);
						// w3m.mol[w3m.global.mol].residueData[atom.chainname][atom.resid].path = tempPathArray;
						// w3m.mol[w3m.global.mol].residueData[atom.chainname][atom.resid].tangents = tempBinormalsArray;
						// w3m.mol[w3m.global.mol].residueData[atom.chainname][atom.resid].normals = tempNormalsArray;
						// w3m.mol[w3m.global.mol].residueData[atom.chainname][atom.resid].binormals = tempTangentsArray;
					// }
				}
                if ( ii == n/2 ) {
					if(w3m.CLENGTH>0){
						...
						// var midpp = w3m.tool.getMidPoint(prep,xyz);
						// var midtan = w3m.tool.getMidAngle(pretan,tan);
						// var midnormal = w3m.tool.getMidAngle(prenormal,normal);
						// var midbinormal = w3m.tool.getMidAngle(prebinormal,binormal);
						// if(midpp){
							// tempPathArray.push(midpp);
							// tempBinormalsArray.push(midbinormal);
							// tempNormalsArray.push(midnormal);
							// tempTangentsArray.push(midtan);
						// }
						w3m.mol[w3m.global.mol].residueData[atom.chainname][atom.resid].path = tempPathArray;
						w3m.mol[w3m.global.mol].residueData[atom.chainname][atom.resid].tangents = tempBinormalsArray;
						w3m.mol[w3m.global.mol].residueData[atom.chainname][atom.resid].normals = tempNormalsArray;
						...
						tempTangentsArray  = [];
						if(prep){
							tempPathArray.push(new THREE.Vector3(prep[0]+offset.x,prep[1]+offset.y,prep[2]+offset.z));
							tempBinormalsArray.push(new THREE.Vector3(prebinormal[0],prebinormal[1],prebinormal[2]));
							tempNormalsArray.push(new THREE.Vector3(prenormal[0],prenormal[1],prenormal[2]));
							tempTangentsArray.push(new THREE.Vector3(pretan[0],pretan[1],pretan[2]));
						}
						prep = xyz;	
						pretan = tan;
						prebinormal = binormal;
						prenormal = normal;						
	...
	getMainAtomById : function(mol_id,id){
        ...
        if(atom !== undefined){
            return {id:atom[1],resid:atom[5],chainname:atom[4]};
        }
        return undefined;
	},
	getMidPoint : function(preP,nextP){
		var offset = PDB.GeoCenterOffset;//偏移量
		if(preP==undefined||nextP==undefined){
			return undefined;
		}
		return new THREE.Vector3((preP[0]+nextP[0])/2+offset.x,(preP[1]+nextP[1])/2+offset.y,(preP[2]+nextP[2])/2+offset.z);	
        
	},
	getMidAngle : function(preP,nextP){
		if(preP==undefined||nextP==undefined){
			return undefined;
		}
		return new THREE.Vector3((preP[0]+nextP[0])/2,(preP[1]+nextP[1])/2,(preP[2]+nextP[2])/2);	
    ...
    var doLaterWork = function () {
        ...
        for ( var i in o.chain ) {
            ...
            if ( w3m_isset(o.helix[i]) ) {
                for ( var ii = 0, ll = o.helix[i].length; ii < ll; ii++ ) {
                    ...
                    for ( var iii = helix[0]; iii <= helix[1]; iii++ ) {
						...
						o.residueData[i][iii].sse = Math.floor((w3m.HELIX-1500)/10);
                    }
                }
            }
            // sheet
                ...
                for ( var j in o.sheet[i] ) {
                    ...
                    for ( var ii = 0, ll = sheet.length; ii < ll; ii++ ) {
                        ...
                        for ( var iii = strand[0]; iii <= strand[1]; iii++ ) {
							...
							o.residueData[i][iii].sse = Math.floor((w3m.SHEET-1500)/10);
                        }
                    }
                }
            }
            ...
            ss_fragment.forEach(function(fg){
				...
				o.residueData[i][fg_start].sse = Math.floor((head-1500)/10);
                for ( var ii = fg[0]+1; ii <= fg[1]-1; ii++ ) {
                    o.ss[i][ii] = [ body, fg_range ];
					o.residueData[i][ii].sse = Math.floor((body-1500)/10);
                }
                o.ss[i][fg_stop] = [ foot, fg_range ];
				o.residueData[i][fg_stop].sse = Math.floor((foot-1500)/10);
            });
        }
    }
    switch ( typeof text ) {
        case 'string' : parse(text); break;
